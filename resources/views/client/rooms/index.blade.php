...
{{-- ============================================================
     ...
     ============================================================ --}}
<div class="relative overflow-hidden" style="background: #0a0a0a; padding-top: 64px; padding-bottom: 80px; min-height: 340px;">
  ...
  {{-- Hero text --}}
  <div class="relative z-10 max-w-6xl mx-auto px-8 pb-14">
    ...
    <h1 class="font-['Playfair_Display'] text-[clamp(2.2rem,5vw,3.5rem)] font-bold text-stone-100 leading-tight">
  ...
  <div class="relative z-20 max-w-6xl mx-auto px-8">
    <form method="GET" action="{{ route('client.rooms.index') }}" id="searchForm">
      <div class="bg-white shadow-xl rounded-xl grid"
        ...
        {{-- Check In --}}
        <div class="px-5 py-4 border-r border-gray-100 hover:bg-gray-50 transition-colors cursor-pointer">
        ...
        {{-- Submit --}}
        <button type="submit"
                class="bg-blue-600 hover:bg-blue-700 text-white text-[11px] font-bold tracking-[0.14em] uppercase px-8 flex items-center justify-center gap-2 transition-colors whitespace-nowrap">
          <i class="fas fa-magnifying-glass text-xs"></i>
          Cập Nhật<br>Tìm Kiếm
        </button>
...
{{-- ============================================================
     ROOM LISTING — big rounded white card, matching Figma
     ============================================================ --}}
<div class="bg-[#f0f2f5] pb-16 pt-6">
  <div class="max-w-6xl mx-auto px-8">
    ...
    <div class="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100">
      ...
      {{-- Card header --}}
      <div class="px-7 pt-7 pb-4 flex items-end justify-between flex-wrap gap-3">
        <div>
          <h2 class="text-2xl font-bold text-gray-900">Hãy chọn phòng</h2>
          <p class="text-sm text-gray-500 mt-0.5">
            @if($roomTypes->count() > 0)
              Hiện có <span class="font-semibold text-gray-700">{{ $roomTypes->count() }} phòng</span>
              phù hợp với bạn &nbsp;·&nbsp;
        ...
        <div class="flex gap-2">
          <button class="flex items-center gap-1.5 px-4 py-2 border border-gray-300 rounded-lg text-gray-600 text-xs font-semibold tracking-wide uppercase hover:border-gray-400 transition-colors">
    ...
    @else
      ...
      {{-- Table --}}
      <div class="mx-7 mb-2 border border-gray-100 rounded-xl overflow-hidden">

        {{-- Column headers --}}
        <div class="hidden md:grid border-b border-gray-100 bg-gray-50 px-5 py-3 text-[10px] font-bold uppercase tracking-widest text-gray-400"
             style="grid-template-columns: 230px 1fr 90px 185px 125px; gap: 1rem;">
        ...
        @foreach($roomTypes as $rt)
          @php
            $dbImgs = $rt->images->pluck('image_url')->toArray();
            $imgs   = count($dbImgs) > 0 ? $dbImgs : ($fallbackImages[$rt->code] ?? $defaultFallback);
            $imgCnt = count($imgs);

            $bedParts = [];
            if ($rt->single_bed_quantity > 0) $bedParts[] = $rt->single_bed_quantity . ' giuong don';
            if ($rt->double_bed_quantity > 0) $bedParts[] = $rt->double_bed_quantity . ' giuong doi';
            ...
            $priceFmt  = number_format((float)$rt->daily_price, 0, ',', '.') . ' &#8363;';
          @endphp

          <div class="grid border-b border-gray-100 last:border-b-0 hover:bg-blue-50/20 transition-colors px-5 py-5 gap-4"
               style="grid-template-columns: 230px 1fr 90px 185px 125px; align-items: start;">

            {{-- 1. Thong tin phong --}}
            <div>
              {{-- Image --}}
              <div class="relative w-full overflow-hidden cursor-pointer group bg-gray-100"
                   style="height: 148px;"
            ...
            <div class="pt-0.5">
              ...
              @empty
                <span class="text-xs text-gray-400">Đang cập nhật...</span>
              @endforelse
              @if($moreCount > 0)
                <div class="text-[11px] text-gray-400 mt-1">+{{ $moreCount }} tien ich khac</div>
              @endif
            </div>
            ...
            <div class="flex items-start flex-wrap gap-0.5 pt-0.5">
              @for($i = 0; $i < $rt->adult_quantity; $i++)
                <i class="fas fa-person text-base text-gray-500" title="Nguoi lon"></i>
              @endfor
              @for($i = 0; $i < $rt->child_quantity; $i++)
                <i class="fas fa-child text-sm text-gray-400" title="Tre em"></i>
              @endfor
            </div>
      ...
      {{-- Total Panel — inside the white card, right-aligned --}}
      <div id="totalSentinel"></div>
      <div id="totalPanel" class="flex justify-end px-7 py-7">
        <div class="bg-gray-50 border border-gray-100 rounded-2xl p-7 w-[320px]">
          <div class="text-[10px] font-bold tracking-widest uppercase text-gray-400 mb-1" id="totalLabel">
            TỔNG GIÁ ({{ $nights }} ĐÊM)
          </div>
          <div class="text-4xl font-bold text-gray-900 mb-5" style="font-family: 'Playfair Display', serif;" id="totalPrice">
            0 &#8363;
          </div>
...
<div id="lightboxOverlay"
  ...
  <div class="absolute top-0 left-0 right-0 flex items-center justify-between px-8 py-5 z-10">
    <span class="text-sm text-white/40 tracking-widest font-medium" id="lbCounter">01 / 01</span>
    <button onclick="closeLightbox()" title="Dong (Esc)"
  ...
  <div class="flex items-center justify-center w-full flex-1 px-20 pt-16 pb-2 gap-6">
    <button id="lbPrev" onclick="lbNavigate(-1)"
            class="flex-shrink-0 w-12 h-12 rounded-full border border-white/20 flex items-center justify-center text-white/50 hover:border-white/60 hover:text-white bg-transparent cursor-pointer transition-all">
  ...
  <div id="lbThumbs" class="flex gap-2 px-8 pb-6 overflow-x-auto justify-center max-w-full" style="scrollbar-width:none;"></div>
...
// ==========================================================
// TOTAL PANEL — Sticky-fixed at bottom until natural position
// ==========================================================
...
function renderLightbox() {
  ...
  if (thumbsWrap) {
    ...
    imgs.forEach(function(src, idx) {
      ...
      div.style.cssText = 'flex-shrink:0;width:68px;height:48px;overflow:hidden;cursor:pointer;border:2px solid ' + (idx===i ? '#d4af37':'transparent') + ';opacity:' + (idx===i ? '1':'0.45') + ';transition:all .2s;';
