...
<div class="flex-1 flex flex-col">
  <div class="p-8 max-w-7xl mx-auto w-full">
    ...
    <div class="flex items-end justify-between mb-8">
      ...
      <a href="{{ route('admin.services.create') }}" class="bg-blue-900 hover:bg-blue-800 text-white px-5 py-2.5 rounded-xl font-bold text-sm flex items-center gap-2 transition-all shadow-lg shadow-blue-900/20 active:scale-95">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-slate-50 border-b border-slate-100">
              ...
              <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Nhóm</th>
              <th class="px-6 py-4 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Giá (VNĐ)</th>
          ...
          <tbody class="divide-y divide-slate-100" id="tableBody">
            ...
            <tr class="hover:bg-slate-50/50 transition-colors group">
              <td class="px-6 py-4">
                <p class="font-bold text-blue-900">{{ $service->id }}</p>
              </td>
              <td class="px-6 py-4">
                <div class="flex items-center justify-center">
                  @if($service->icon)
                    <span class="material-symbols-outlined text-2xl text-slate-600">{{ $service->icon }}</span>
                  @else
              ...
              <td class="px-6 py-4">
                <p class="font-bold text-slate-900">{{ number_format($service->unit_price ?? 0, 0, '.', '.') }}đ</p>
              </td>
              <td class="px-6 py-4">
                <div class="flex items-center justify-center gap-2">
                  <a href="{{ route('admin.services.edit', $service->id) }}" class="p-2 text-slate-400 hover:text-amber-500 hover:bg-amber-50 rounded-lg transition-all" title="Chỉnh sửa">
                    <span class="material-symbols-outlined text-xl">edit</span>
                  </a>
                  <button class="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-all delete-service-btn" title="Xóa" data-service-id="{{ $service->id }}" data-service-name="{{ $service->name }}">
            ...
            <tr>
              <td colspan="6" class="px-6 py-12 text-center">
                <p class="text-slate-500 text-sm">Chưa có tiện ích nào. <a href="{{ route('admin.services.create') }}" class="text-blue-900 font-bold hover:underline">Thêm tiện ích mới</a></p>
              </td>
            </tr>
            @endforelse
          </tbody>
      ...
      <div class="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
        <p class="text-xs text-slate-500 font-medium">
          Hiển thị {{ $services->count() }} / {{ $services->total() }} tiện ích
        </p>
