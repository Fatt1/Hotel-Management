        ...
        <div class="flex flex-col gap-2">
          ...
          >
            ...
            @foreach($groups as $group)
              <option value="{{ $group->id }}" {{ old('group_id', $service->group_id) == $group->id ? 'selected' : '' }}>
                {{ $group->service_name }}
              </option>
            @endforeach
          </select>
        ...
        <div class="flex flex-col gap-2">
          ...
          @if(old('icon', $service->icon))
            <div class="flex items-center gap-2 p-2 bg-blue-50 border border-blue-200 rounded-lg">
              <span class="material-symbols-outlined text-2xl text-blue-900">{{ old('icon', $service->icon) }}</span>
              <span class="text-sm text-blue-900 font-medium">Đã chọn: {{ old('icon', $service->icon) }}</span>
            </div>
          @endif
        </div>
