...
<div class="px-6 py-6">
    ...
    <div id="checkout-meta" class="hidden"

         data-booking-id="{{ $booking->id }}"

         data-already-paid="{{ $alreadyPaid }}"></div>
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <div class="flex items-center gap-2 mt-0.5">
                    ...
                    <span class="bg-blue-100 text-blue-700 text-xs px-2 py-0.5 rounded font-medium flex items-center gap-1">
        ...
        <form action="{{ route('admin.bookings.checkout.confirm', $booking->id) }}" method="POST">
    ...
    <div class="flex flex-col lg:flex-row gap-6 items-start">
        ...
        <div class="bg-white border border-slate-200 rounded-2xl p-6">
                    ...
                    @foreach($booking->bookingDetails as $index => $detail)
                        ...
                        @php
                            ...
                            $servicesJson  = $detail->serviceUsages->map(fn($u) => [
                        ...
                        <div class="room-checkout-card border border-slate-200 rounded-xl mb-4 transition-all hover:border-blue-300 hover:shadow-sm"

                             data-index="{{ $index }}"

                             data-room-name="P.{{ $detail->room->name }}"

                             data-room-type="{{ $detail->room->roomType->name }}"

                             data-room-charge="{{ $roomCharge }}"
                                ...
                                <div class="grid grid-cols-12 items-center gap-1">
                                    ...
                                    <div class="col-span-5">
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <span class="font-black px-2 py-1 rounded-lg text-sm min-w-[2.5rem] text-center {{ $badgeColor }}">
                                            ...
                                            <div class="min-w-0">
                                                ...
                                                <div class="font-bold text-slate-900 flex items-center gap-1 flex-wrap text-sm">

                                                    P.{{ $detail->room->name }}

                                                    @if($isEarlyCI)

                                                        <span class="text-[9px] bg-purple-100 text-purple-600 px-1.5 py-0.5 rounded font-bold uppercase tracking-wide">CI sớm</span>

                                                    @endif

                                                    @if($isLateCO)

                                                        <span class="text-[9px] bg-red-100 text-red-600 px-1.5 py-0.5 rounded font-bold uppercase tracking-wide">CO muộn</span>
                                                ...
                                                <div class="text-[11px] text-gray-400 mt-0.5 truncate">
                                                    ...
                                                    {{ $checkinDate->format('H:i d/m') }} -> {{ $checkoutDate->format('H:i d/m/Y') }}

                                                </div>

                                                <div class="text-[11px] text-blue-400 font-semibold">{{ $nights }} đêm</div>
                                    ...
                                    <div class="col-span-3 text-xs text-gray-500 font-medium">{{ $detail->room->roomType->name }}</div>
                            ...
                            <div class="bg-slate-50 px-4 py-3 rounded-b-xl border-t border-slate-100 space-y-1">
                                ...
                                @if($detail->serviceUsages->isNotEmpty())
                                    ...
                                    <div class="text-[10px] font-bold text-gray-400 uppercase mb-1.5 flex items-center gap-1">

                                        <span class="material-symbols-outlined text-[12px]">room_service</span>  Dịch vụ đã dùng
                                    ...
                                    <div class="flex justify-between text-xs text-gray-500">
                                        ...
                                        <span class="font-medium">{{ number_format($usage->unit_price * $usage->quantity, 0, ',', '.') }}đ</span>
        ...
        <div class="relative bg-white border border-slate-200 rounded-2xl p-6">
            ...
            <div id="invoice-loading" class="hidden absolute inset-0 z-10 flex items-center justify-center bg-white/70 rounded-2xl">
            ...
            <div id="invoice-content">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-4">
                    {{-- Tiền phòng --}}
                    <div>
                        <h3 class="text-[11px] font-bold text-gray-400 uppercase mb-2 tracking-wider">Tiền phòng</h3>
                        <div class="space-y-1.5 text-sm" id="invoice-rooms-list"></div>
                        <div class="flex justify-between text-xs font-semibold text-slate-600 mt-2 pt-2 border-t border-slate-200">
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-6 space-y-4">
                ...
                <div>
                    <div class="flex justify-between items-center mb-2">
                        <span class="text-[11px] font-bold text-green-600 uppercase tracking-wider">Đã thanh toán</span>
                        <span class="text-sm font-bold text-green-600">- {{ number_format($alreadyPaid, 0, ',', '.') }}đ</span>
                    </div>
                        ...
                        <div class="flex justify-between items-start text-xs">
                            <div>
                                ...
                                <div class="text-gray-400 mt-0.5">
                                    ...
                                    @if($payment->staff) · {{ $payment->staff->first_name }} {{ $payment->staff->last_name }}@endif
                                </div>
                            </div>
                            <span class="text-green-600 font-semibold whitespace-nowrap ml-3">+ {{ number_format($payment->amount, 0, ',', '.') }}đ</span>
                        </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="bg-slate-50 rounded-xl border border-slate-200 p-4 space-y-3">
                    ...
                    <div class="flex justify-between items-center pt-1 border-t border-slate-200">
                        <span class="text-xs font-semibold text-gray-500 uppercase tracking-wide" id="remaining-label">Còn lại</span>
